 ...
 * @subpackage  Jshoppingorder.Wishboxtransferusertogoup
 */
namespace Joomla\Plugin\Jshoppingorder\Wishboxautolabels\Extension;
...
{	
	...
	{
		return [
			'onAfterchangeProductQTYinStock'    => 'onAfterchangeProductQTYinStock'
		];
	}
	 ...
	 *
	 */
	public function onAfterchangeProductQTYinStock(Event $event)
	{
		$item = $event->getArgument(0);
		$autolabelsModel = JSFactory::getModel('autolabels', 'Site\\Wishbox');
		if (!$autolabelsModel->update([$item->product_id])) {
